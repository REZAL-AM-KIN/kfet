    ...
    const ProduitsList = () => {
        ...
        return (
            produits.map((produit) => {
                return (<option key={produit.id}
                                value={produit.id}>{produit.raccourci} | {produit.nom} - {produit.prix}€</option>)
            })
        );
    }
        ...
        <Container fluid className="d-flex justify-content-center">
            <Col md={6} className="me-4">
                <Container className="mb-2">
                    <Row>
                        ...
                        }} onKeyPress={handleDebucquer} value={selectedProduit} autoFocus size={10}>
                            <ProduitsList/>
                        </select>
                    </Row>
                </Container>
            ...
            <Col md={4}>
                <Stack gap={1} className="">
                    <Button onClick={() => {
                        handleDebucquer()
                    }}>Débucquer (Entrée)</Button>
                    <Button onClick={() => {
                        setShowQrModal(true)
                    }}>Scan QR (Alt+L)</Button>
                    <RechargeButton/>
                    <Button>annuler??</Button>
                    <Button onClick={() => {
                        handleQuitter()
                    }}>Quitter (Alt+X)</Button>
                    <CategorieList/>
                </Stack>
            </Col>
            ...
            <Modal show={showRechargeModal} size="lg" centered onHide={() => setShowRechargeModal(false)}
                   onEntered={() => {
                       montantRechargeRef.current.focus()
                   }}>
                <Modal.Header closeButton>
                ...
                <Modal.Body>
                    <form onSubmit={handleRechargerSubmit}>
                        ...
                        <input
                            ...
                            required/>
                        <select id="methode" value={methode} onChange={(e) => {
            ...
            <Modal show={showQrModal} size="lg" centered onHide={() => setShowQrModal(false)} onEntered={() => {
                montantQrRef.current.focus()
            }}>
                <Modal.Header closeButton>
