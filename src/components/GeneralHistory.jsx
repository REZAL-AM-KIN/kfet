...
function History(props) {
  ...
  const lines = result.map((line, key) => {
    ...
    var date_to_print = "";
    if (ago.getDay() >= 7) {
      date_to_print = date.toLocaleString("fr-fr", { month: "long", day: "numeric", hour: "numeric", minute: "numeric" })
    } else {
        ...
        <td onClick={() => window.location.href = "/pg/" + line.cible_evenement.id}>
          <Card style={{ borderRadius: '1rem' }} bg={"light"} border={+(line.cible_evenement.solde) >= 0 ? "success" : "danger"} text={+(line.cible_evenement.solde) >= 0 ? "success" : "danger"} >
            <Card.Body>
              ...
              <Card.Subtitle className="fw-normal lh-1 text-nowrap">{line.cible_evenement.nom} {line.cible_evenement.prenom}</Card.Subtitle>
            </Card.Body>
          </Card>
        </td>
