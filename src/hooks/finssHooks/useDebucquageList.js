...
/*
Permet de récupérer une liste des débucquages pour un fin'ss (toutes les participations bucquées, et celle débucquée si
display_debucquee est true)
...
Retours:
    ...
    retrieve: Fonction qui force l'actualisation de bucquages
    page: page courante
...
export function useDebucquageList(finssId, initial_limit=25) {
    ...
    const retrieve = useCallback(async () => {
        setLoading(true)
        try {
            const response = await axiosPrivate.get("event_debucquage/", {params:
                    {finss:finssId, limit:limit, offset:(page-1)*limit, ordering:ordering, search:search, displayDebucquee:displayDebucque}});
            if (response.data) {
                setBucquages(response.data.results);
                setNumberRecords(response.data.count);
            } else {
                errorNotif("Débucquage","Impossible de récupérer les bucquages du Fin'ss");
            }
        } catch (error) {
            errorNotif("Débucquage", error.message)
            console.log("Error getting Prebucquages", error);
        }
        setLoading(false)
    }, [axiosPrivate, finssId, limit, page, ordering, search, displayDebucque])
    ...
    return {bucquages, isLoading, retrieve, page, setPage, limit, setLimit, ordering, setOrdering, search, setSearch,
        displayDebucque, setDisplayDebucque, numberRecords}
