...
/*
Permet de récupérer une liste des prébucquages pour un fin'ss (toutes les participations avec prebucque_quantity>0)
...
export function usePrebucquageList(finssId, initial_limit=25) {
    const axiosPrivate = useAxiosPrivate()

    const [isLoading, setLoading] = useState(true)
    const [prebucquages, setPrebucquages] = useState([])
    ...
    const retrieve = useCallback(async () => {
        setLoading(true)
        try {
            const response = await axiosPrivate.get("event_prebucquage/", {params:
                    {finss:finssId, limit:limit, offset:(page-1)*limit, ordering:ordering, search:search}});
            if (response.data) {
                setPrebucquages(response.data.results);
                setNumberRecords(response.data.count);
            } else {
                errorNotif("Prébucquage","Impossible de récupérer les prébucquages du Fin'ss");
            }
        } catch (error) {
            errorNotif("Prébucquage", error.message)
    ...
    return {prebucquages, isLoading, retrieve, page, setPage, limit, setLimit, ordering, setOrdering, search, setSearch,
        numberRecords}
