...
export function useEntiteManageablesList(limit){
    ...
    const retrieveEntities = useCallback( async () => {
            setLoading(true)
            try {
                const response = await axiosPrivate.get("mesentitesmanageables/",
                    {params: {limit:limit, offset:(page-1)*limit, search:search}});
                if (response.data) {
            ...
            setLoading(false)
        }, [axiosPrivate, limit, page, search]
    )
    ...
    return {entitiesList, isLoading, retrieveEntities, pageCount, page, setPage, search, setSearch}
